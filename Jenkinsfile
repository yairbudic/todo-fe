pipeline {
    ...
    stages {
        ...
        stage('Delivery') {
            steps {
                echo 'Delivery....'
                sh 'DOCKER_BUILDKIT=1 docker build -f Dockerfile-pipeline -t ybudic/todo-fe:${BUILD_NUMBER} -t ybudic/todo-fe:latest --target delivery .'
            }
        }
         stage('Cleanup') {
            steps {
         ...
         stage('Push') {
            steps {
                ...
                sh 'docker push  --all-tags ybudic/todo-fe:${BUILD_NUMBER}'
            }
        }
    }
}